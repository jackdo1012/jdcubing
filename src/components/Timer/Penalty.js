...
function Penalty(props) {
  ...
  const resetMiniStats = () => {
    dispatch(
      getFirstMiniStats(localStorage.getItem(`times${session}`).split(','))
    )
    dispatch(
      getSecondMiniStats(localStorage.getItem(`times${session}`).split(','))
    )
  }
  useEffect(() => {
  ...
  const handleDnf = () => {
    ...
    resetMiniStats()
    dispatch(
      getNumberOfSolves(localStorage.getItem(`times${session}`).split(','))
    )
    dispatch(getBest(localStorage.getItem(`times${session}`).split(',')))
    dispatch(getWorst(localStorage.getItem(`times${session}`).split(',')))
  }
