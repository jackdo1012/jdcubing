  ...
  useEffect(() => {
    ...
    if (run === 'stop') {
      if (scrambleList === null) {
        ...
        localStorage.setItem('scramble', [scramble].concat(scrambleList))
      }
    }
  }, [run])
  useEffect(() => {
      ...
      switch (scrambleType) {
        ...
        case '4': {
          ...
          for (let i = 0; i < newFormatScramble.length; i++) {
            if (newFormatScramble[i] === newFormatScramble[i].toLowerCase()) {
              if (newFormatScramble[i].includes("'")) {
                newFormatScramble[i] =
                  newFormatScramble[i].substring(0, 1).toUpperCase() + "w'"
              } else if (newFormatScramble[i].includes('2')) {
                newFormatScramble[i] =
                  newFormatScramble[i].substring(0, 1).toUpperCase() + 'w2'
              } else {
                newFormatScramble[i] = newFormatScramble[i].toUpperCase()
              }
            }
          }
        ...
        case 'oh': {
          ...
          let newScram = new Scrambo().type(`333`).length(20).get(1).join(' ')
        ...
        case 'mega': {
          ...
          let newScram = new Scrambo().type(`minx`).length(77).get(1).join(' ')
          dispatch(submitScramble({ type: 'mega', scramble: newScram }))
          setScramble(newScram)
          break
        }
        case 'pyra': {
          document.querySelector('.scramble-text').id = 'pyra'
          let newScram = new Scrambo().type(`pyram`).length(11).get(1).join(' ')
          dispatch(submitScramble({ type: 'pyra', scramble: newScram }))
          setScramble(newScram)
          break
        }
        case 'ske': {
          document.querySelector('.scramble-text').id = 'ske'
          let newScram = new Scrambo().type(`skewb`).length(8).get(1).join(' ')
          dispatch(submitScramble({ type: 'ske', scramble: newScram }))
          setScramble(newScram)
          break
        }
        default: {
          ...
          <div className="selection">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                localStorage.setItem('scramble-type', e.target.value)
              }}
              onFocus={() => {
                if (window.matchMedia('(min-width: 1024px)').matches) {
                  document.querySelector('.selection select').blur()
                }
              }}
            >
